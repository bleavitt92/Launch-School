...
# or longer way
def gender_male?(hash)
  hash['gender'] == 'male'
end 

man_hash = munsters.select do |k, hash|
  gender_male?(hash)
end 

ages = []
man_hash.each do |name, value_hash|
  ages << value_hash['age']
end 
ages.sum
# ============================================================================
...
# Given this previously seen family hash, print out the name, age and gender of each family member: (Name) is a (age)-year-old (male or female).
...
arr.map do |hash|
  hash.select do |key, values|
    values.all? do |num|
      num.even?
    end 
  end 
end 
# wrong solution above because was not seeing that arr[1] was one big hash
...
def UUID
  letters = ('a'..'f').to_a
  nums = ('0'..'9').to_a
  hex_arr = nums + letters
  hex_arr.sample(8).join + '-' + hex_arr.sample(4).join + '-' + hex_arr.sample(4).join + '-' + hex_arr.sample(4).join + '-' + hex_arr.sample(12).join
end 
...
def generate_UUID2
  ...
  lengths.each_with_index do |length, index|
    ...
    uuid += '-' unless index >= length.size - 1 
  end 
  uuid
end 